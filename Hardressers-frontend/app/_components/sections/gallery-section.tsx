    ...
    <section id="Gallery" className="bg-black px-4 py-16 sm:px-6 sm:py-24">
      <div className="mx-auto max-w-7xl">
        ...
        {editMode && isAdmin ? (
          ...
          <>
            <CarouselShell itemsLength={visiblePhotos.length}>
              {visiblePhotos.map((photo, idx) => (
                <div
                  key={photo.id}
                  data-card
                  onClick={() => setLightboxIndex(idx)}
                  className="group relative aspect-[3/4] w-[85%] shrink-0 snap-start cursor-zoom-in overflow-hidden rounded-2xl border border-[#D4AF37]/20 bg-[#0a0a0a] transition-all duration-300 hover:border-[#D4AF37]/50 sm:w-[calc(50%-8px)] lg:w-[calc(33.333%-16px)]"
                >
                  <ImageWithFallback
                    src={photo.photoUrl}
