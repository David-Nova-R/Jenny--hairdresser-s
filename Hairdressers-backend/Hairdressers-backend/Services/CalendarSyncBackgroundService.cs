        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            while (!stoppingToken.IsCancellationRequested)
            {
                try
                {
                    ...
                    await appointmentService.CompletePassedAppointmentsAsync();
                    _logger.LogInformation("Sync Google Calendar et complétion automatique effectuées à {Time}", DateTime.Now);
                }
                catch (Exception ex)
                {
